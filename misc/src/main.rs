...
fn main() {
    ...
    selection_sort(&mut sort_vec, true);
    println!("{:?}", sort_vec);

    selection_sort(&mut sort_vec, false);
    println!("{:?}", sort_vec);
...
// If initial value x is positive or zero, then abs(x) == x

fn test_fabs_nonneg_equality() {
    
}

// If initial value x is negative, abs(x) > x
